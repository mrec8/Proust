...
class MetricsTracker:
    ...
    def log_skill_created(self, skill_description: str, task: str, commands: List[str]) -> None:
        """
        Log when a new skill is acquired.
        
        Args:
            skill_description: Name or description of the skill
            task: Task that led to acquiring the skill
        """
        self.metrics["cumulative_metrics"]["skills_acquired"] += 1
        self.metrics["acquired_skills"].append({
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "skills_acquired": self.metrics["cumulative_metrics"]["skills_acquired"],
    ...
    def print_summary(self) -> None:
        ...
        print(f"  skills Acquired: {summary['skills_acquired']}")
        print("="*50)
    ...
    def plot_metrics(self, save_path: Optional[str] = None) -> None:
        ...
        # Plot 3: Cumulative skills acquired
        skill_times = [entry["timestamp"] for entry in self.metrics["acquired_skills"]]
        skill_indices = list(range(1, len(skill_times) + 1))
        
        if skill_indices:  # Only plot if there are skills
            axs[1, 0].step(skill_times, skill_indices)
            axs[1, 0].set_title('skills Acquisition Over Time')
            axs[1, 0].set_xlabel('Time (s)')
            axs[1, 0].set_ylabel('Cumulative skills')
    ...
    def _extract_objects(self, observation: str) -> List[str]:
        ...
        for obj in common_objects:
            if obj in observation_lower.split():
